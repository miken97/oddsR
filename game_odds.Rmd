...
```{r}
# totals
total <- lambda_away + lambda_home

totals_fn <- function(par, total, away_ml, home_ml, under, over) {
  
  lambda_away <- par[1]
  lambda_home <- par[2]
  
  joint_pdf_df <- crossing(
...
away_win_pr <- mean(rpois(10000, lambda = mle_fit$par[1]) > rpois(10000, lambda = mle_fit$par[2])) 
home_win_pr <- mean(rpois(10000, lambda = mle_fit$par[1]) < rpois(10000, lambda = mle_fit$par[2])) 

away_win_pr / (away_win_pr + home_win_pr)
home_win_pr / (away_win_pr + home_win_pr)
...
joint_pdf_totals |> 
  select(goals_away, goals_home, joint_pdf) |> 
  mutate(
    ml_winner = case_when(
      goals_away > goals_home ~ "away",
      goals_away < goals_home ~ "home",
  ...
  group_by(winner) |> 
  summarize(prob = sum(joint_pdf))
  
  pivot_wider(names_from = goals_home, values_from = joint_pdf)
```
